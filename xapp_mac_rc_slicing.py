...
class MACCallback(ric.mac_cb):
    ...
    # Required for storing past 400 CQI values for predicting the next CQI value
    input = np.empty(400)
    ...
    # PRB vs CQI Mapping
#    prb_cqi_map = np.array([92,37,16,10,8,6,6,5,4,4,3,3,3,3,2])
    prb_cqi_map = np.array([365, 148, 64, 38, 30, 24, 21, 17, 15, 13, 11, 11, 9, 9, 8])
        ...
        if len(ind.ue_stats) > 0:
           if (ind.ue_stats[0].frame != self.prev_frame):
              ...
              # Once the input is ready, predict the CQI value of upcoming frame everytime
                 ...
                 try:
                    self.pred_log = self.pred_log + '['+str(ind.ue_stats[0].frame).rjust(4,'0') + ',' + str(int(self.input[-1])).rjust(2, '0') + ',' + str(int(self.pred_CQI)).rjust(2, '0') + ',' + str(self.prb_cqi_map[int(self.pred_CQI)-1]).rjust(2, '0') + '],   '
                    if(self.pred_count%9==0): 
...
#                          print('Time Elapsed: '+ str(round(time.time()-self.t,2)) + ' Sec, Frame: ' + str(self.prev_frame) + ', Accuracy: '+ str(acc) + '%, MAE: ' + str(error_mae) + ', MSE: ', str(error_mse))
                          ...
                          print('------------------------------------------------------------------------------------')
                          self.pred_log = '\n\nFrame Level Predictions [Frame Number, Actual CQI, Predicted CQI, Guaranteed PRBs]\n\n'
                    ...
                    msg = fill_slice_ctrl_msg("ASSOC_UE_SLICE", assoc_ue_slice,self.prb_cqi_map[int(self.pred_CQI)-1],(ind.ue_stats[0].frame+1)%1024)
...
def fill_slice_ctrl_msg(ctrl_type, ctrl_msg, nPRB, fn):
